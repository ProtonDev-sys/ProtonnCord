...
val bytecodeVer = libs.versions.bytecodeVersion.get()
...
data class JsTool(val kind: String, val exe: File) {
    val installCommand: List<String>
        get() = when (kind) {
            "bun" -> listOf(exe.absolutePath, "install", "--frozen-lockfile", "--silent")
...
fun findJsTool(): JsTool? {
    ...
    fun candidatesFor(kind: String): List<File> = buildList {
        if (!windows) pathDirs.forEach { add(File(it, kind)) }
        when (kind) {
            "bun" -> {
                homes.forEach { add(File(it, ".bun/bin/bun")) }
                if (windows) System.getenv("APPDATA")?.let {
                    add(File(it, "npm/node_modules/bun/bin/bun.exe"))
                }
            }
...
configure(subprojects.filter { it.path.startsWith(":plugins:") }) {
    ...
    extensions.configure<LibraryExtension> {
        ...
        compileOptions {
            sourceCompatibility = JavaVersion.toVersion(bytecodeVer)
            targetCompatibility = JavaVersion.toVersion(bytecodeVer)
        }
        sourceSets {
    ...
    extensions.configure<KotlinAndroidProjectExtension> {
        ...
        compilerOptions.jvmTarget.set(JvmTarget.fromTarget(bytecodeVer))
    }
    ...
    tasks.register("dexJar") {
        ...
        doLast {
            ...
            val d8 = File(com.android.tools.r8.D8::class.java.protectionDomain.codeSource.location.toURI())
            ...
            execOps.exec {
                commandLine(
                    java.absolutePath, "-cp", d8.absolutePath, "com.android.tools.r8.D8",
                    "--min-api", minSdkVer,
...
val buildJs = tasks.register("buildJs") {
    ...
    doLast {
        val tool = findJsTool()
        requireNotNull(tool) { "Install Bun or pass -PjsTool=/-PjsToolPath= to build the required JS bundle." }
