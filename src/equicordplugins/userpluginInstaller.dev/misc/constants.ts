...
export const CLONE_LINK_REGEX = /https:\/\/(?:((?:git(?:hub|lab)\.com|git\.(?:[a-zA-Z0-9-]+\.)+[a-zA-Z]{2,63}|codeberg\.org))\/(?!user-attachments)((?:[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,98}[a-zA-Z0-9])?))\/((?:[a-zA-Z0-9](?:[a-zA-Z0-9._-]{0,98}[a-zA-Z0-9])?))(?:\.git)?|(plugins\.(nin0)\.dev)\/((?:[a-zA-Z0-9](?:[a-zA-Z0-9._-]{0,98}[a-zA-Z0-9])?)))(?:\/)?/;
...
export function restartAfterPluginChange(native: boolean) {
    return (native ? relaunch() : reload()).catch(error => Alerts.show({
...
export function showInstallFinishedAlert(pluginToEnable: string, native: boolean) {
    Alerts.show({
        ...
        onConfirm() {
            ...
            return restartAfterPluginChange(native);
        },
        onCancel: () => restartAfterPluginChange(native)
    });
}
