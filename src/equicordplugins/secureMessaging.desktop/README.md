...
Opening a message fetches supported image/video/audio previews and any detached text. Small document/media-only bundles also preview documents as inert `text/plain` blobs after full-bundle authentication. This requires an authenticated filename ending in `.txt`, `.log`, `.md`, `.json`, `.csv`, `.yaml`, `.yml`, `.toml`, `.ini`, or `.xml`, no spoiler documents or detached text, and at most 256 KiB of total ciphertext. The wire preview flags remain unchanged, so older manifests stay valid. Other files, including ZIPs, executables, and documents outside these limits, are fetched, authenticated, decrypted, and saved only after a download click; ZipPreview's expander is hidden for these deferred files. Older bundles lack an external authenticated file list, so they show generic file rows and require full-bundle authentication on download. Older detached text also needs the full bundle.
